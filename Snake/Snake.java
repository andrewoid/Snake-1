...
/**
 * Contains a list of Square objects that represent the snake. <br/>
 * The snake moves by taking the last square on its tail and inserting it in
 * front of the head, in the direction that it is going, then deleting the tail.
 */
class Snake implements Iterable<Square> {
	 ...
	 * 
	 * @param direction
	 *            the direction to direction the Snake.
	 * @return true if the Snake did not collide with itself
	 */
	private synchronized boolean move(Direction direction) {
		...
		if (contains(head))
			return false; // If snake collided with itself.
	 ...
	 * 
	 * @param x
	 *            the number of tiles to grow the snake by
	 */
	 ...
	 * 
	 * @param sq
	 *            the specified Square to check for
